...
define('JV', JV15 ? 'j1' : 'j2');
define('ROOT', dirname(__FILE__));

require_once ROOT . '/installer/helper_' . JV . '.php';

		cleanupInstall();

		// Install the Installer languages
		installLanguages(ROOT . '/installer/language', 1, 0);
		...
		if (JV == 'j1' && function_exists('beforeInstall_j1')) {
			beforeInstall_j1($db);
		} else if (JV == 'j2' && function_exists('beforeInstall_j2')) {
			beforeInstall_j2($db);
		...
		if (JV == 'j1' && function_exists('afterInstall_j1')) {
			afterInstall_j1($db);
		} else if (JV == 'j2' && function_exists('afterInstall_j2')) {
			afterInstall_j2($db);
		} else if (function_exists('afterInstall')) {
			afterInstall($db);
		}

		if (!installFiles(ROOT . '/extensions')) {
	...
	function folder_copy($src, $dest, $force = 0)
	{
		...
		if (!JFolder::exists($dest) && !folder_create($dest)) {
			...
			$app->enqueueMessage(JText::sprintf(JText::_('NNI_FAILED_TO_CREATE_DIRECTORY'), $folder), 'error error_folders');
			$success = 0;
		}
			...
			foreach ($files as $file) {
				...
				if ($force || !JFile::exists($file_dest)) {
					if (!$ftp->store($file_src, $file_dest)) {
						...
						$app->enqueueMessage(JText::sprintf(JText::_('NNI_ERROR_SAVING_FILE'), $file_path), 'error error_files');
						$success = 0;
					}
				}
			}
