...
defined('_JEXEC') or die;
	...
	function installFiles($folder)
	{
		...
		if (JFolder::exists($folder . '/' . JV)) {
			if (!copy_from_folder($folder . '/' . JV, 1)) {
				return 0;
			}
		}
		if (JFolder::exists($folder . '/' . JV . '_optional')) {
			if (!copy_from_folder($folder . '/' . JV . '_optional', 0)) {
				return 0;
			}
		}
	...
	function installExtension($states, $alias, $name, $type = 'component', $extra = array(), $reinstall = 0)
	{
		...
		if ($installed && $alias != 'nnframework') {
			...
			if ($version) {
				...
				if (!(strpos($version, 'PRO') === false)) {
					...
					JLog::add(JText::_('NNI_ERROR_PRO_TO_FREE') . '<br /><br />' . html_entity_decode(JText::sprintf('NNI_ERROR_UNINSTALL_FIRST', $url, $n)), JLog::ERROR);
					return -1;
					...
					JLog::add(JText::_('NNI_ERROR_BEFORE_SWITCH') . '<br /><br />' . html_entity_decode(JText::sprintf('NNI_ERROR_UNINSTALL_FIRST', $url, $n)), JLog::ERROR);
					return -1;
				}
			}
		}
		...
		if (!$installed) {
			if ($type == 'module') {
				...
				if (!$row->store()) {
					JLog::add($row->getError(), JLog::ERROR);
					return 0;
				}
	...
	function installFramework()
	{
		...
		if ($do_install) {
			if (!installFiles($framework_folder)) {
				JLog::add(JText::_('NNI_COULD_NOT_INSTALL_THE_NONUMBER_FRAMEWORK_EXTENSION'), JLog::ERROR);
				JLog::add(JText::_('NNI_COULD_NOT_COPY_ALL_FILES'), JLog::ERROR);
				$success = 0;
			}
			if ($success) {
