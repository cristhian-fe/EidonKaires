...
class plgSystemModalizerHelper
{
	...
	function placeScriptsJQuery()
	{
		if ($this->params->load_jquery) {
			JFactory::getDocument()->addScript(JURI::root(true) . '/media/modalizer/modals/jquery.min.js');
		} else {
			JFactory::getDocument()->addScriptDeclaration('$j = jQuery.noConflict();');
		}
	}
	...
	{
		JFactory::getDocument()->addScript(JURI::root(true) . '/media/modalizer/modals/colorbox/jquery.colorbox-min.js');
		if (is_int($this->params->modal_colorbox_style)) {
			$this->params->modal_colorbox_style = 'style' . $this->params->modal_colorbox_style;
		}
		JFactory::getDocument()->addStyleSheet(JURI::root(true) . '/media/modalizer/modals/colorbox/' . $this->params->modal_colorbox_style . '/colorbox.css');
	...
	function handleLink($link)
	{
		...
		preg_match('#^[^>]*>#si', $link, $tag);
		$tag = $tag['0'];

		$hasclass = preg_match('#\sclass="(.*?)"#si', $tag, $c);
		$hastarget = preg_match('#\starget="(.*?)"#si', $tag, $t);
		$hasrel = preg_match('#\srel="(.*?)"#si', $tag, $r);
		...
		if ($ismodal) {
			...
			if ($isindex) {
				$this->addML($url);
				$tag = str_replace($u['0'], 'href="' . $url . '"', $tag);
			}

			// remove onclick actions
			$tag = preg_replace('#\sonclick=".*?"#si', '', $tag);

			if ($hastarget) {
				$tag = str_replace($t['0'], ' target="_blank"', $tag);
			} else {
			...
			if ($rel) {
				if ($hasrel) {
					$tag = str_replace($r['0'], ' rel="' . trim($rel) . '"', $tag);
				} else {
					$tag = str_replace('<a ', '<a rel="' . trim($rel) . '" ', $tag);
				}
			}

			if ($hasclass) {
				$tag = str_replace($c['0'], ' class="' . trim($class) . '"', $tag);
			} else {
				$tag = str_replace('<a ', '<a class="' . trim($class) . '" ', $tag);
			}

			$link = str_replace($tag_orig, $tag, $link);
		}
	...
	{
		if (!preg_match('#\{' . preg_quote($this->params->syntax, '#') . '#s', $html)) {
			return;
		}

		// remove tags within alt and titles of tags
		$html = preg_replace('#(title|alt)="\{' . preg_quote($this->params->syntax, '#') . '(?:\s+[^\}]*)?\}(.*?)\{/' . preg_quote($this->params->syntax, '#') . '\}"#s', '\1="\2"', $html);

		// handle modal tags within other links (like in menu items)
