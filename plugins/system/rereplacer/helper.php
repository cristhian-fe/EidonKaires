...
class plgSystemReReplacerHelper
{
	...
	function prepareRegex(&$str, $dot, $case)
	{
		$str = '#' . $str . '#';
		if ($dot) {
			$str .= 's'; // . (dot) also matches newlines
		}
		if (!$case) {
			$str .= 'i'; // case-insensitive pattern matching
		}
	...
	function replaceVariables(&$str)
	{
		if (!(strpos($str, '[[user:') === false)) {
			if (preg_match_all('#\[\[user\:([^\]]+)\]\]#', $str, $matches, PREG_SET_ORDER) > 0) {
				if (!$this->params->user) {
					...
					self::flattenParams($this->params->user);
				}
				foreach ($matches as $match) {
					if ($match['1'] == 'password' || $match['1']['0'] == '_') {
						...
						if (!$this->params->contact) {
							...
							$query->select('c.*')
								->from('#__' . $this->params->contact_table . ' as c')
								->where('c.user_id = ' . (int) $this->params->user->id);
							$db->setQuery($query);
							$this->params->contact = $db->loadObject();
							if ($this->params->contact) {
								self::flattenParams($this->params->contact);
							} else {
								$this->params->contact = new stdClass();
								$this->params->contact->x = '';
							}
						}
						if (!$this->params->profile) {
							...
							$query->select('p.profile_key, p.profile_value')
								...
								->where('p.user_id = ' . (int) $this->params->user->id);
							$db->setQuery($query);
						...
						} else if (isset($this->params->profile->{$match['1']})) {
							$str = str_replace($match['0'], $this->params->profile->{$match['1']}, $str);
						} else {
							$str = str_replace($match['0'], '', $str);
						}
					}
		...
		if (!(strpos($str, '[[random:') === false)) {
			while (preg_match('#\[\[random\:([0-9]+)-([0-9]+)\]\]#', $str, $match)) {
				...
				if ($pos !== false) {
					$str = substr_replace($str, $replace, $pos, strlen($match['0']));
				}
			}
		}
	...
	function filterItems($items, $article = 0)
	{
		...
		foreach ($items as $item) {
			if (
				...
				|| (JFactory::getDocument()->getType() == 'feed' && $item->enable_in_feeds == 0)
